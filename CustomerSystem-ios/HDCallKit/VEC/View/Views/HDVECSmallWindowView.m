...
@property (nonatomic, strong) HDVECCollectionViewCell *currentCell;
@end
...
- (BOOL)setThirdUserdidJoined:(HDVECCollectionViewCellItem *)item{
    ...
    @synchronized(_cellArray){
        for (HDVECCollectionViewCellItem * tt in _cellArray) {
            if (tt.uid  == item.uid) {
                isNeedAdd = NO;
                break;
            }
...
//        flowLayout.sectionInset = UIEdgeInsetsMake(0, 25, 5, 25);//上左下右
        ...
        [_collectionView registerClass:[HDVECCollectionViewCell class] forCellWithReuseIdentifier:@"HDCallCollectionViewCell"];
        //设置代理
...

{
    ...
    HDVECCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:identify forIndexPath:indexPath];
    [cell sizeToFit];
...

{
    ...
    self.currentCell = (HDVECCollectionViewCell *)  [self.collectionView cellForItemAtIndexPath:indexPath];
