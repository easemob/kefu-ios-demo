...
#import "HDVECSKTagView.h"
#import "HDVECSKTagButton.h"


@interface HDVECSKTagView ()
...
- (void)onTag: (HDVECSKTagButton *)btn {
    
    if (self.hdvecDidTapTagAtIndex) {
        self.hdvecDidTapTagAtIndex([self.subviews indexOfObject: btn]);
    }
    if (self.hdvecDidTapTagAtButton) {
        self.hdvecDidTapTagAtButton( btn,btn.tag);
    }
}

#pragma mark - Public

- (void)addTag: (HDVECSKTag *)tag {
    NSParameterAssert(tag);
    HDVECSKTagButton *btn = [HDVECSKTagButton buttonWithTag: tag];
    btn.tag = tag.tagId;
...
- (void)insertTag: (HDVECSKTag *)tag atIndex: (NSUInteger)index {
    NSParameterAssert(tag);
