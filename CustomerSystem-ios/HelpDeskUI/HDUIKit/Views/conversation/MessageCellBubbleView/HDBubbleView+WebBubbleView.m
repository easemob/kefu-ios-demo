...
#import "KFWebModel.h"
@implementation HDBubbleView (WebBubbleView)
...
- (void)setupHtmlBubbleView{
    ...
    [self _setupHtmlBubbleConstraints];
    
    NSLog(@"===进来了===%@",self.textLabel.text);
    
}
- (void)_setupHtmlBubbleConstraints
{
    [self.webView hdmas_remakeConstraints:^(MASConstraintMaker *make) {
...
//
//    json = @"{\"type\": \"richtext\",\"content\": \"http://image.qiniu.zhenligroup.com/FhXy0pEQFQzh9VIjJpOnFNIa-XAj\"}";
    KFWebModel * webModel = [KFWebModel yy_modelWithJSON:json];
...
{
    CGSize retSize = CGSizeMake(240, 500);
   
    return CGSizeMake(retSize.width + 11 * 2 + 9, retSize.height);
