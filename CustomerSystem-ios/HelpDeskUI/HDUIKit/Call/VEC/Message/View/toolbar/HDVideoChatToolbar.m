...
#import "HDEmotionEscape.h"
@interface HDVideoChatToolbar()<UITextViewDelegate, HDFaceDelegate>
...
@end
@implementation HDVideoChatToolbar
...
//    HDChatToolbarItem *moreItem = [[HDChatToolbarItem alloc] initWithButton:self.moreButton withView:self.moreView];
    ...
    imageButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    [imageButton setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/chatBar_colorMore_photo"] forState:UIControlStateNormal];
    [imageButton setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/chatBar_colorMore_photo"] forState:UIControlStateSelected];
    [imageButton addTarget:self action:@selector(imageButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    // 把录音按钮和自定义的录音按钮所在的控件newRecordView传进去，从而显示自己的view
    HDChatToolbarItem *styleItem = [[HDChatToolbarItem alloc] initWithButton:imageButton withView:self.recordView];
    [self setInputViewLeftItems:@[styleItem]];
    //send
       self.sendButton = [[UIButton alloc] init];
       self.sendButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
       [self.sendButton setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/hd_type_select_btn_nor"] forState:UIControlStateNormal];
       [self.sendButton setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/hd_type_less_btn_nor"] forState:UIControlStateSelected];
       [self.sendButton addTarget:self action:@selector(sendButtonAction:) forControlEvents:UIControlEventTouchUpInside];
       HDChatToolbarItem *sendItem = [[HDChatToolbarItem alloc] initWithButton:self.sendButton withView:self.moreView];
...
//        }
        for (HDChatToolbarItem *item in self.rightItems) {
            if (item.button == self.sendButton) {
                item.button2View = _moreView;
                break;
            }
        }
