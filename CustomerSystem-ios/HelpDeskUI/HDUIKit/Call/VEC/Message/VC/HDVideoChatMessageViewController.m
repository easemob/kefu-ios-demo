...
@interface HDVideoChatMessageViewController ()<HDMessageCellDelegate,HDChatManagerDelegate,TransmitDeleteTrackMsgDelegate, UIGestureRecognizerDelegate,HRobotUnsolveItemViewDelegate>
{
...
@implementation HDVideoChatMessageViewController
{
    NSString *_title;
}
...
- (void)viewDidLoad {
    ...
    //Initialization
    CGFloat chatbarHeight = [HDVideoChatToolbar defaultHeight];
    
    self.videoChatToolbar = [[HDVideoChatToolbar alloc] initWithFrame:CGRectMake(0, self.view.hd_height - chatbarHeight - iPhoneXBottomHeight, self.view.hd_width, chatbarHeight)];
...
- (void)setVideoChatToolbar:(HDVideoChatToolbar *)videoChatToolbar
{
    ...
    if ([videoChatToolbar isKindOfClass:[HDVideoChatToolbar class]]) {
        [(HDVideoChatToolbar *)self.videoChatToolbar setDelegate:self];
        self.chatBarMoreView = (HDChatBarMoreView*)[(HDVideoChatToolbar *)self.videoChatToolbar moreView];
        self.faceView = (HDFaceView*)[(HDVideoChatToolbar *)self.videoChatToolbar faceView];
        // 初始化录音按钮所在的view
        self.recordView = (HDRecordView *)[(HDVideoChatToolbar *)self.videoChatToolbar recordView];
        // 添加代理
        self.recordView.delegate = self;
    }
}

- (void)setDataSource:(id<HDVideoChatMessageViewControllerDataSource>)dataSource
{
...
- (void)setDelegate:(id<HDVideoChatMessageViewControllerDelegate>)delegate
{
    _delegate = delegate;
}
    ...
    void (^refresh)(NSArray *messages) = ^(NSArray *messages) {
        dispatch_async(_messageQueue, ^{
            ...
            hd_dispatch_main_async_safe(^(){
                HDVideoChatMessageViewController *strongSelf = weakSelf;
                if (strongSelf) {
...
- (void)didSelectTakePicAction:(UIButton *)imageButton{
    ...
    [[HDAgoraCallManager shareInstance].currentVC presentViewController:self.imagePicker animated:YES completion:NULL];
