...
@interface HDVECIDCardScaningView () {
...
@implementation HDVECIDCardScaningView
...
- (void)setVideoScanType:(HDVECIDCardScaningViewType)type withISSmallWindow:(BOOL)iSSmallWindow{
    ...
    switch (type) {
        case HDVECIDCardScaningViewTypeIDCard:
            // 添加扫描窗口
            [self addScaningWindow];
            break;
        case HDVECIDCardScaningViewTypeFace:
            // 添加扫描窗口 人像
...
-(void)addFaceScaningWindow {
    ...
    headIV.image = [UIImage imageNamed:@"HelpDeskUIResource.bundle/idcard_first_head"];
