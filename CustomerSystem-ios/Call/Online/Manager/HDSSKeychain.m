...
NSString *const kHDSSKeychainErrorDomain = @"com.samsoffes.hdsskeychain";

NSString *const kHDSSKeychainAccountKey = @"acct";
NSString *const kHDSSKeychainCreatedAtKey = @"cdat";
NSString *const kHDSSKeychainClassKey = @"labl";
NSString *const kHDSSKeychainDescriptionKey = @"desc";
NSString *const kHDSSKeychainLabelKey = @"labl";
NSString *const kHDSSKeychainLastModifiedKey = @"mdat";
NSString *const kHDSSKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE
CFTypeRef HDSSKeychainAccessibilityType = NULL;
#endif

@interface HDSSKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation HDSSKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = HDSSKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
        *error = [NSError errorWithDomain:kHDSSKeychainErrorDomain code:status userInfo:nil];
        return nil;
    }
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
        if (HDSSKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
    return HDSSKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
    ...
    if (HDSSKeychainAccessibilityType) {
        CFRelease(HDSSKeychainAccessibilityType);
    }
    HDSSKeychainAccessibilityType = accessibilityType;
}
#endif
