...
@interface HDVECAnswerCallBackView()
{
    NSInteger _time ;
...
@implementation HDVECAnswerCallBackView
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        //创建ui
        self.backgroundColor = [[HDVECAppSkin mainSkin] contentColorBlockalpha:1];
        [self _creatUI];
    }
    return self;
}
...
- (UIButton *)onBtn{
    if (!_onBtn) {
        ...
        //为button赋值
        [_onBtn setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/on.png"] forState:UIControlStateNormal];
...
- (UIButton *)offBtn{
    if (!_offBtn) {
        ...
        //为button赋值
        [_offBtn setImage:[UIImage imageNamed:@"HelpDeskUIResource.bundle/off.png"] forState:UIControlStateNormal];
    }
    return _offBtn;
}
