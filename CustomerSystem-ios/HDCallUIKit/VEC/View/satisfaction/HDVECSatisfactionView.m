...
#import "HDVECEnquiryOptionModel.h"
#import "HDVECEnquiryTagModel.h"
#import "MBProgressHUD+Add.h"
#import "HDVECAgoraCallManager.h"
#import "HDVECAppSkin.h"
@interface HDVECSatisfactionView()<UITextViewDelegate,CWStarRateViewDelegate,HDVECSatisfactionEvaluationTagSelectDelegate>
{
    HDMessage *_model;
...
@property (nonatomic,strong) HDVECSKTagView *tagView;;
@property (nonatomic,strong) NSMutableArray  *currentTags;
@property (nonatomic,strong) UIView *maskView;
@end
@implementation HDVECSatisfactionView
...
- (void)setEnquiryInvite:(NSDictionary *)enquiryInvite withModel:(nonnull HDMessage *)message{
    ...
    if ([[enquiryInvite allKeys] containsObject:@"enquiryOptions"] && [[enquiryInvite valueForKey:@"enquiryOptions"] isKindOfClass:[NSArray class]]) {
        ...
        NSArray *enquiryOptions = [NSArray yy_modelArrayWithClass:[HDVECEnquiryOptionModel class] json:[enquiryInvite valueForKey:@"enquiryOptions"]];
...
- (void)initData{
    ...
    HDVECEnquiryOptionModel * m3 = [self getConfigModel:@"EnquiryCommentEnable"];
    ...
    // 获取EnquiryInviteMsg
    HDVECEnquiryOptionModel * m1 = [self getConfigModel:@"EnquiryInviteMsg"];
    self.textLabel.text = m1.optionValue;
   
    // 获取EnquiryDefaultShow5Score
    HDVECEnquiryOptionModel * m4 = [self getConfigModel:@"EnquiryDefaultShow5Score"];
...
- (HDVECEnquiryOptionModel *)getConfigModel:(NSString *)tagname{
    
    __block HDVECEnquiryOptionModel * model;
    [_enquiryOptions enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        
        HDVECEnquiryOptionModel * m = obj;
...
- (void )hd_calculateAppraiseTags:(NSArray *)appraiseTags{
    ...
    [appraiseTags enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        HDVECEnquiryTagModel * model = obj;
        [self.currentTags addObject:model];
        HDVECSKTag *tag = [[HDVECSKTag alloc] initWithText:model.tagName];
        tag.tagId = [model.tagId integerValue];
        tag.font = [UIFont systemFontOfSize:14];
        tag.textColor = [[HDVECAppSkin mainSkin] contentColor555555];
        tag.bgColor = [[HDVECAppSkin mainSkin] contentColorF0F0F0];
        tag.cornerRadius = 10;
        tag.enable = YES;
...
-(void)getEnquiryTagData:(NSString *)score{
    ...
    NSArray *tags = [NSArray yy_modelArrayWithClass:[HDVECEnquiryTagModel class] json:array];
    if (tags &&tags.count > 0) {
        ...
        kWeakSelf
        self.evaluationTagView.hdvecDidTapTagAtButton = ^(UIButton *button, NSUInteger buttonId) {
            
            button.selected = !button.selected;
        
            if (button.isSelected) {
                [button setTitleColor: [[HDVECAppSkin mainSkin] contentColorBlueHX] forState: UIControlStateNormal];
//                button.titleLabel.textColor = [[HDAppSkin mainSkin] contentColorBlueHX] ;
                button.backgroundColor = [[HDVECAppSkin mainSkin] contentColorE2EDFD];
                [weakSelf clickEvaluationTagSelectWithArray:buttonId WithSelect:YES];
                
            }else{
//                button.titleLabel.textColor = [[HDAppSkin mainSkin] contentColorBlueHX] ;
                [button setTitleColor: [[HDVECAppSkin mainSkin] contentColor555555] forState: UIControlStateNormal];
                button.backgroundColor = [[HDVECAppSkin mainSkin] contentColorF0F0F0];
...
- (UILabel*)titleLabel
{
    if (_titleLabel == nil) {
        ...
        _titleLabel.textColor =[[HDVECAppSkin mainSkin] contentColor555555];
...
- (UILabel*)textLabel
{
    if (_textLabel == nil) {
        ...
        _textLabel.textColor  =[[HDVECAppSkin mainSkin] contentColor555555];
...
- (UILabel*)msgLabel
{
    if (_msgLabel == nil) {
        ...
        _msgLabel.textColor =[[HDVECAppSkin mainSkin] contentColor555555];
...
- (UILabel *)evaluateTitle
{
    if (_evaluateTitle == nil) {
        ...
        [_evaluateTitle setTextColor:[ [HDVECAppSkin mainSkin] contentColorF2AC3C]];
...
- (UITextView*)textView
{
    if (_textView == nil) {
        ...
        _textView.layer.borderColor =  [[HDVECAppSkin mainSkin] contentColorF0F0F0].CGColor;
        _textView.layer.cornerRadius = 5.f;
        _textView.layer.borderWidth = 0.5;
        _textView.textColor  =[[HDVECAppSkin mainSkin] contentColor555555];
        _textView.backgroundColor = [[HDVECAppSkin mainSkin] contentColorF0F0F0];
        ...
        UILabel *placeHolderLabel = [[UILabel alloc] init];
               ...
               placeHolderLabel.textColor = [[HDVECAppSkin mainSkin] contentColorBCBCBC];
...
//        [_commitBtn setTitle:@"提交评价" forState:UIControlStateNormal];
        ...
        [_commitBtn setBackgroundColor:[[HDVECAppSkin mainSkin] contentColorBlueHX]];
...
- (void)commit{
    ...
    HDVECEnquiryOptionModel *model = [self getConfigModel:optionName];
    ...
    HDVECEnquiryOptionModel *model1 = [self getConfigModel:@"EnquiryCommentEnable"];
    HDVECEnquiryOptionModel *model2 = [self getConfigModel:commentName];
    ...
    [[HDClient sharedClient].callManager vec_submitVisitorEnquirySessionid:rtcSessionId withScore:score withComment:comment withTagData:self.evaluationTagsArray Completion:^(id  _Nonnull responseObject, HDError * _Nonnull error) {
        ...
        if (error == nil) {
            ...
            // 获取EnquiryInviteMsg
            HDVECEnquiryOptionModel * m2 = [self getConfigModel:@"EnquirySolveMsg"];
...
- (void)clickEvaluationTagSelectWithArray:(NSInteger)tagId  WithSelect:(BOOL)select
{
    ...
    for (int i = 0; i < self.currentTags.count; i ++) {
        ...
        HDVECEnquiryTagModel * model = self.currentTags[i];
